...
const TABLE_WHITELIST = new Set([
  ...
  'admin_user_relation', 'invite_link', 'admin_quota_pool', 'visit_log',
])
...
class PgQueryBuilder {
  ...
  async runInsert() {
    ...
    const sql = `INSERT INTO public.${this.table} (${keys.join(', ')}) VALUES (${ph}) RETURNING *`
  ...
  async runUpdate() {
    ...
    const sql = `UPDATE public.${this.table} AS ${this.alias} SET ${sets.join(', ')}${where} RETURNING ${this.alias}.*`
  ...
  async runUpsert() {
    ...
    const sql = `INSERT INTO public.${this.table} (${keys.join(', ')}) VALUES (${ph})
      ON CONFLICT (${this.upsertConflict}) DO UPDATE SET ${updates} RETURNING *`
    const { rows } = await db.query(sql, values)
    return wrapResult(rows, rows.length, null, this.singleMode, this.maybeSingleMode)
  }
  ...
  async runDelete() {
    const params = []
    const where = this.buildWhere(params)
    const ret = this.singleMode || this.maybeSingleMode ? ' RETURNING *' : ''
