...
3. 实习/工作中明确出现的公司名必须写入公司字段，不得整段塞进描述后留空。$prompt$,
    now(), now()
  ),
  (
    'project_optimize',
...
2. 优先保留原文事实；描述过短时可按岗位做克制补全，不得编造虚假公司或量化业绩。$prompt$,
    now(), now()
  ),
  (
    'jd_resume_optimize',
    $prompt$按岗位要求优化完整简历：优先使用候选人自己简历中的真实信息，再结合岗位内容做衍生改写。
1. 保留姓名、联系方式、教育、已有公司名、项目名与日期；用岗位术语规范表达、重排技能、重写评价，并逐条优化已有经历。
2. 只在已有经历框架内做合理衍生，禁止凭空虚构新公司、新项目或精确量化业绩；不能写得过于虚假。
3. 相对原文须有实质提升，但以真实可面试为准，不为「看起来完美」而编造。$prompt$,
    now(), now()
  ),
  (
    'pdf_jd_optimize',
    $prompt$结合简历原文与岗位要求优化完整简历。
1. 优先保留原文事实专有名词，再按岗位对齐职责表述与技能呈现。
2. 基于已有经历衍生改写，禁止过度虚构；不得编造原文没有的公司、项目或数字成果。$prompt$,
    now(), now()
  ),
  (
    'jd_image_extract',
