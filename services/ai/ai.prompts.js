...
Step 11 输出说明：写入 optimization_notes 或 custom_fields 中的披露信息（见各任务输出约束）。`
...
const COMMON_JD_ALIGNMENT_RULES = `## JD 对齐与重塑规则（用于按岗位优化）
1. 先从 JD 提取：岗位名称、核心职责、必须项、加分项、硬技能/工具、学历/经验门槛与成果期待。
2. 允许把简历「适当优化成该岗位所需的样子」：用 JD 术语规范同义表达，重排 skills，重写 summary，把各段经历改写为更能证明 JD 要求的职责与交付（基于已有事实深化，而非整段无关抄袭 JD）。
3. 用户已提供的真实公司、学校、项目名、日期、联系方式必须保留；不得把 JD 里的甲方公司写成候选人就职公司。
4. JD 明确要求但简历完全无相关依据的硬门槛（如必须持有某证书），不得写成已具备；可在 optimization_notes 建议补充。
5. 弱相关经历可压缩篇幅，但不要无故删除整段任职造成时间断档。
6. 必须逐条处理 resume 中已有的项目/实习/工作；禁止只改 summary 忽略经历。`
...
若触发极简输入补全，必须在 custom_fields 中加入一项：
{"label":"生成说明","value":"由于提供信息过少，已基于意向岗位生成若干示意性基本信息，请按真实经历修改后再投递。"}`
...
- optimization_notes：字符串数组，3-5条，必须对应本次真实改动（如：重写了个人评价、按岗位重排技能、优化了第N段工作职责等）。
...
3. 仅有姓名+意向岗位（或其他字段基本为空）时：必须执行极简输入补全，生成评价、技能、示意公司经历与项目，并写入 custom_fields 生成说明。
4. 不得输出几乎空白、无法投递的简历。`,
...
const JD_RESUME_OPTIMIZE_PROMPT = composePrompt(
  ...
  COMMON_JD_ALIGNMENT_RULES,
  `## 当前任务：按岗位JD优化结构化完整简历
解析JD后，把简历适当优化成该岗位所需的样子：重写 summary、重排并补强 skills、逐条优化 projects/internships/work_experiences 的每一条。
1. JD岗位名清晰唯一时，resume.target_position 使用该岗位名；否则保留原目标。
2. 保留姓名、联系方式、教育、已有真实公司名与日期；重点升级表达与岗位对齐。
3. 输入几乎只有姓名+岗位时：执行极简输入补全，并在 optimization_notes 首条披露。
4. 禁止优化后与输入几乎无差异；optimization_notes 必须写清改动点。`,
  COMMON_FULL_RESUME_WORKFLOW,
  COMMON_EVIDENCE_RULES,
  COMMON_BALANCED_CONSTRUCTION_RULES,
  COMMON_EXPERIENCE_RULES,
...
先忠实提取PDF事实，再按JD重塑表达与模块侧重；逐条优化全部经历。
保留真实专有名词与联系方式；极简PDF时允许示意补全并披露。
禁止无实质改动。`,
  COMMON_FULL_RESUME_WORKFLOW,
  COMMON_EVIDENCE_RULES,
  COMMON_BALANCED_CONSTRUCTION_RULES,
...
2. 信息过少时可基于岗位生成示意内容并明确提示用户修改。`,
  jd_resume_optimize: `按岗位JD优化完整简历，适当重塑为岗位所需样子。
1. 重写评价、技能与每一条经历；保留真实姓名、联系方式、教育与已有公司名。
2. 信息过少时基于岗位生成示意内容，并在优化说明首条提示用户按真实经历修改。
3. 禁止优化后几乎无变化。`,
  pdf_jd_optimize: `结合简历原文与岗位JD优化完整简历。
1. 保留原文事实专有名词，按岗位对齐表达并逐条优化经历。
2. 信息过少时允许示意补全并披露；禁止无实质改动。`,
