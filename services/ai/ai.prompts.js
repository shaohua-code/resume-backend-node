...
4. 原字段为空但存在目标岗位或相关上下文时，不得机械返回空内容；应生成与现有信息不冲突的最低可用版本。原描述只有一句时，除规范术语外，还应补足合理的动作、对象、方法或交付物，体现实质优化，而不是只替换同义词。
5. 构造内容必须与候选人阶段相符：学生/校招生侧重专业基础、课程与项目参与；实习/初级侧重执行、协助和交付；中高级只有在履历证据支持时才体现复杂度、独立性、管理或业务影响。`;
// ========== 全局经历表达规则 ==========
...
Step 5 summary重建：不是简单润色原文，而是基于全简历重建职业画像，回答“是谁、核心能力、为何匹配、差异化优势”。有足够证据时写2-4个紧凑短句、通常60-120字；包含岗位导向适度构造规则时，信息较少也应结合目标岗位和现有字段生成最低可用summary；只有目标岗位和全部职业信息均为空时才填""。禁止空泛自评和求职口号。
...
2. ATS：有明确目标岗位时名称准确且原样，核心关键词使用标准名称，并在summary、skills和对应经历间形成自然关联；没有明确目标时target_position保持空，不猜测岗位或强做匹配。由B级有界扩展或当前任务允许的适度构造形成的基础能力可以出现在summary/skills；适度构造也可用于补足已有经历的空白描述，但不得伪装成强事实、量化成果或重复计分证据；无关键词堆砌、隐藏词或同义反复。
...
const RESUME_GENERATE_PROMPT = composePrompt(
  ...
  当用户字段较少时仍要生成可使用的精简简历：存在明确目标岗位或任一有效教育、技能、项目、实习、工作信息时，应尽量重建summary、skills及已有经历的描述，不因原字段为空而直接放弃；但不得为了完整而制造强事实。`,
...
const LAZY_GENERATE_PROMPT = composePrompt(
  ...
  完成事实提取后，按适度构造规则补足低风险内容。即使自由文本很短，只要存在明确目标岗位或任一有效教育/经历信息，也应生成精简但可用的summary、skills和相关经历表达，不得只是把原文搬进JSON或让主要内容全部为空。`,
  COMMON_FULL_RESUME_WORKFLOW,
  COMMON_EVIDENCE_RULES,
  COMMON_BALANCED_CONSTRUCTION_RULES,
  COMMON_EXPERIENCE_RULES,
...
1. 原始描述为空时，优先使用项目名称、本人角色、技术栈、目标岗位和简历中的相关技能，合理构造1-2条最低可用描述；信息仍很少时可补充该类项目常见且低风险的参与动作、实现过程或交付物。
2. 原始描述过短时，补足动作、对象、方法/工具和交付物，通常生成2-3条；素材较丰富时生成3-5条。
3. 可以补充符合项目类型和候选人阶段的常见背景、实现过程、协作及交付表达，但不得编造具体业务名称、技术版本、系统规模、量化指标或明确成果。
optimized使用“\\n”分隔各要点；highlights输出0-4条从optimized概括出的岗位亮点，不得比optimized作出更强主张。只要存在目标岗位、项目字段或相关简历上下文之一，就不应机械返回空字符串。`,
  COMMON_INPUT_BOUNDARY,
  `## 输入数据
<target_position>{target_position}</target_position>
<project_record>
...
原summary是否为空不影响任务，必须优先依据完整简历重新生成。只要存在明确目标岗位或任一有效教育、专业、课程、技能、项目、实习、工作、证书信息，就生成2-3个紧凑短句，通常60-100字；证据较少时可根据目标岗位补充克制的基础能力、可参与工作和发展方向，不得直接返回空字符串。
...
1. 原始描述为空时，根据公司、实习岗位、目标岗位及简历已有技能，合理生成1-2条该岗位常见且低风险的参与内容或交付表达。
2. 原始描述过短时，补足工作对象、动作、协作或交付物，通常生成2-3条；信息充分时可生成3-4条。
3. 可适当构造符合实习生阶段的执行、协助、资料整理、基础分析、功能实现、内容制作、客户/业务支持等内容，但具体采用哪类内容必须由实际岗位决定，不得默认技术岗。
optimized使用“\\n”分隔各要点；highlights输出0-3条从optimized概括的岗位亮点，不得新增更强事实。只要存在目标岗位、实习记录字段或相关上下文之一，就不应机械返回空字符串。`,
  COMMON_INPUT_BOUNDARY,
  `## 输入数据
<target_position>{target_position}</target_position>
<internship_record>
...
1. 原始描述为空时，根据公司、职位、部门、目标岗位和简历已有能力，合理构造2-3条该岗位常见职责、协作方式和交付内容；原始描述过短时通常扩展为2-4条，信息充分时生成3-5条。
2. 允许加入与岗位相符的低风险价值表达，如“支持业务推进、完善日常流程、协助问题处理、保障任务按要求交付”，但不得虚构具体改进结果、量化业绩、业务规模或客户评价。
3. 只有输入明确时才写从0到1、管理、跨部门牵头、项目管理、技术影响力或持续优化，不得为显得资深而拔高。
optimized使用“\\n”分隔各要点；highlights输出0-4条从optimized概括的岗位亮点，不得作出更强主张。只要存在目标岗位、工作记录字段或相关上下文之一，就不应机械返回空字符串。`,
  COMMON_INPUT_BOUNDARY,
  `## 输入数据
<target_position>{target_position}</target_position>
