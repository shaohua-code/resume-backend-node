...
5. target_position只提取原文明示的求职意向、意向岗位、目标岗位或应聘岗位；原文出现「求职意向：xxx」时必须写入 target_position（值为 xxx），不得留空，也不得改用中文键名输出。没有明确岗位时输出空字符串。
...
8. 出现两所及以上学校、专升本、续本、第二学历时，必须拆成多条 educations 对象；禁止把多所学校写进同一 school/major 字段。即使第二段缺少独立时间，也要单独成条。
 ...
 * 只描述如何处理输入与内容质量，不含输出格式/字段名等程序契约。
 * 真正调用时若无 DB 覆盖，仍使用上方完整 FULL Prompt，保证行为不变。
 */
const CODE_DEFAULT_INSTRUCTIONS = {
  resume_generate: `根据用户填写的结构化信息生成完整、可投递的简历。
1. 联合全部字段建立候选人画像，明确求职方向；目标岗位为空时不要臆造岗位名称。
2. 优先保留姓名、联系方式、公司、学校、项目名、日期等明确事实，仅做规范与重排。
3. 描述过短或为空时，可按岗位常识补充低风险的常规职责与能力表述；不得虚构公司、客户、数字、年限、证书或量化业绩。
4. 不得凭岗位新增用户未填写的项目、实习或工作经历；软技能用经历证明，不堆砌空话。
5. 结果需便于招聘初筛阅读，并经得住面试追问。`,
...
3. 删除流水账与套话，每条要点只表达一个核心证据。`,
  summary_optimize: `优化个人评价，形成清晰、真实、可面试的能力画像。
1. 结合目标岗位与全简历重写，回答“是谁、核心能力、为何匹配、差异化优势”。
2. 只使用简历中已有证据，禁止空泛自评、口号式表述和无法面试解释的夸张。
3. 原文评价可吸收其中不冲突的有效信息，再按岗位相关性重组。`,
  skills_optimize: `整理并优化技能列表，提升岗位相关度与可检索性。
1. 统一标准名称、去重，按与目标岗位的相关度排序。
2. 只保留有经历或输入依据的硬技能、工具、方法与资质；软技能不作为技能标签堆砌。
3. 不得凭岗位臆造未掌握的框架、版本或精通级表述。`,
  internship_optimize: `优化单条实习经历描述，突出真实参与范围与交付物。
1. 结合目标岗位与全简历上下文，把职责写清楚：做了什么、服务对象、使用工具、产出什么。
2. 不得编造业务结果、好评或量化提升；贡献程度保持协助/参与/负责等准确表述。
3. 删除无关流水账，保留可面试追问的事实。`,
  work_experience_optimize: `优化单条工作经历描述，强化职责、动作与有依据的价值。
1. 结合目标岗位与全简历，把职责升级为清晰的任务、个人动作、方法工具与交付表达。
2. 有明确业务结果才写结果；不得虚构规模、比例、管理人数或因果业绩。
3. 保持职业阶段一致，避免把协助写成主导。`,
  jd_match: `分析简历与岗位要求的匹配度，给出可执行的对照结论。
1. 先提取岗位核心职责、必须项、加分项与硬门槛，再与简历证据逐项对照。
2. 明确区分直接匹配、可迁移匹配、简历未体现与不适用；未体现不等于候选人不会。
3. 指出主要优势与关键缺口，缺口只能建议补充证据，不得写成已具备能力。
4. 岗位信息过短或矛盾时，不做伪精确结论。`,
  score: `按通用评分口径为简历打分，并给出简短、可落地的改进建议。
1. 综合内容完整度、技能相关性、经历证据质量、结构可读性与文本规范进行评估。
2. 不因敏感信息缺失、学校名气或关键词堆砌加减分。
3. 建议应具体指向可修改处，避免空泛口号。`,
  pdf_optimize: `基于上传简历原文优化完整简历内容，并给出简短优化说明。
1. 以原文明确事实为准，提升岗位相关性、可读性与信息密度。
2. 不得虚构强事实或新增原文不存在的经历；说明需对应本次实际修改。`,
  jd_resume_optimize: `按岗位要求优化完整简历，使内容更贴合目标岗位。
1. 以现有简历事实为边界，用岗位术语规范同义表达并强化相关证据。
2. 可补充与已有信息紧密相关的低风险常规表述，不得把岗位硬门槛写成已具备能力。
3. 同步给出与本次实际修改对应的简短说明；无实质修改时说明可为空。`,
  pdf_jd_optimize: `结合简历原文与岗位要求优化完整简历。
1. 优先保留原文事实，再按岗位对齐职责表述、技能名称与证据呈现。
2. 不得编造原文没有的公司、项目、数字或成果；缺口只能建议补充。`,
  jd_image_extract: `忠实转录图片中的岗位招聘信息为纯文本。
1. 按原文顺序尽量完整转录，不润色、不补全、不推断缺失内容。
2. 看不清或截断处如实留空或标注无法辨认，不得猜测填写。`,
};
