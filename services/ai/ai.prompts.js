 ...
 * 3. 基于 JD 的整份优化：可按岗位要求重塑表达与模块侧重，仍须保留姓名、联系方式、真实教育与已有公司名等明确事实（极简输入除外）。
...
const COMMON_JD_ALIGNMENT_RULES = `## JD 对齐与衍生规则（用于按岗位优化）
核心原则：先用简历里已有的真实信息，再结合 JD 做岗位化衍生改写；优先真实，克制虚构，不能写成「另一个人的履历」。
1. 先从 JD 提取：岗位名称、核心职责、必须项、加分项、硬技能/工具、学历/经验门槛与成果期待。
2. 再盘点简历已有事实（公司、职位、项目、技能、教育、日期、描述）。优化时必须以这些事实为底稿：用 JD 术语规范同义表达、重排 skills、重写 summary、把已有经历改写得更贴合岗位职责与交付。
3. 「衍生」只允许：在已有公司/项目/职位框架内，补全或改写合理且可面试解释的职责表达；可补充与已有技能紧密相邻、基础且低风险的能力表述。禁止凭空新编整段虚假公司、虚假项目、虚假客户、精确量化业绩或未掌握的硬门槛资质。
4. 用户已提供的真实公司、学校、项目名、日期、联系方式必须保留；不得把 JD 里的招聘方公司写成候选人就职公司。
5. JD 明确要求但简历完全无相关依据的硬门槛，不得写成已具备；可在系统锁定的优化说明中建议补充。
6. 弱相关经历可压缩篇幅，但不要无故删除整段任职造成时间断档。
7. 必须逐条处理 resume 中已有的项目/实习/工作；禁止只改 summary 忽略经历。
8. 仅当简历几乎只有姓名（及空经历）时，才可做最低限度的岗位示意补全，且必须披露信息过少；只要简历已有任何真实公司/项目/描述，就禁止再虚构新的示意公司或示意项目覆盖真实履历。`
...
- optimization_notes：字符串数组，3-5条，必须由你总结本次实际改动/生成亮点（面向用户可读），例如重写了个人评价、按岗位补齐技能、优化了某段工作职责等。禁止由前端或模板套话代替。
若触发极简输入补全，optimization_notes 第一条必须是：
「由于提供信息过少，已基于意向岗位生成若干示意性基本信息，请按真实经历修改后再投递。」
禁止输出空的 optimization_notes；禁止写「无实质修改」。不得输出markdown、解释、标题、分析过程或额外字段；description中的换行必须正确JSON转义。`
...
const JD_RESUME_OPTIMIZE_PROMPT = composePrompt(
  ...
  COMMON_JD_ALIGNMENT_RULES,
  `## 当前任务：按岗位JD优化结构化完整简历
把「候选人自己的简历」与「目标岗位JD」结合起来优化：简历事实优先，岗位要求用于对齐表达与侧重点，禁止写成与本人经历无关的虚假履历。
1. JD岗位名清晰唯一时，resume.target_position 使用该岗位名；否则保留原目标。
2. 必须保留姓名、联系方式、教育、已有真实公司名、项目名与日期。
3. 对已有 summary、skills、projects、internships、work_experiences 逐条审阅并岗位化改写；空描述可基于该条已有公司/职位/项目名做克制补全。
4. 禁止在已有真实履历之外再虚构新公司、新项目；禁止编造精确量化业绩。
5. 相对输入须有实质表达提升，但不能为了「看起来完美」而过度虚假。`,
  COMMON_FULL_RESUME_WORKFLOW,
  COMMON_EVIDENCE_RULES,
  COMMON_BALANCED_CONSTRUCTION_RULES,
  COMMON_EXPERIENCE_RULES,
...
const PDF_JD_OPTIMIZE_PROMPT = composePrompt(
  ...
  COMMON_MUST_DIFF_RULES,
  COMMON_JD_ALIGNMENT_RULES,
  `## 当前任务：联合PDF原文与JD优化完整简历
先忠实提取PDF中的履历事实，再按JD对齐表达：优先PDF真实信息，结合岗位要求衍生改写，禁止过度虚构。
保留真实专有名词与联系方式；逐条优化已有经历；不得用示意公司/项目覆盖PDF已有事实。`,
  COMMON_FULL_RESUME_WORKFLOW,
  COMMON_EVIDENCE_RULES,
  COMMON_BALANCED_CONSTRUCTION_RULES,
  COMMON_EXPERIENCE_RULES,
...
2. 优先保留原文事实；描述过短时可按岗位做克制补全，不得编造虚假公司或量化业绩。`,
  jd_resume_optimize: `按岗位要求优化完整简历：优先使用候选人自己简历中的真实信息，再结合岗位内容做衍生改写。
1. 保留姓名、联系方式、教育、已有公司名、项目名与日期；用岗位术语规范表达、重排技能、重写评价，并逐条优化已有经历。
2. 只在已有经历框架内做合理衍生，禁止凭空虚构新公司、新项目或精确量化业绩；不能写得过于虚假。
3. 相对原文须有实质提升，但以真实可面试为准，不为「看起来完美」而编造。`,
  pdf_jd_optimize: `结合简历原文与岗位要求优化完整简历。
1. 优先保留原文事实专有名词，再按岗位对齐职责表述与技能呈现。
