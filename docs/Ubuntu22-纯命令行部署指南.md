...
### Q4: `DB FAIL: Connection terminated due to connection timeout`

**最常见原因**：`.env` 里 `DATABASE_URL` 写了公网 IP `175.178.62.55`。后端与数据库在同一台机器时，必须连 `127.0.0.1`，且安全组也不应开放 5432。
...
# 应只有一行，且 host 为 127.0.0.1
sudo systemctl status postgresql
psql -h 127.0.0.1 -U ai_resume -d ai_resume -c "SELECT 1"
```
...
若 `psql -h 127.0.0.1` 也失败，先处理 PostgreSQL（见 Q1、Q2），不要改 Node 代码。

### Q4b: `DB FAIL: Connection terminated unexpectedly`

同 Q4，检查 `DATABASE_URL` 是否用 `127.0.0.1`，用户名/库名是否为 `ai_resume`（下划线，不是连字符）。
